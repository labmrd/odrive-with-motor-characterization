...
time = (data(:,1) / 8000);                %[s]
voltage = data(:,2);                      %[V]
pos = (data(:,3) - data(1,3)) / motorCPR; %[rev]
vel = data(:,4) * 60 / motorCPR;          %[rpm]
...
ylabel('Position [rev]');
xlim(plotRange);
ylim([min(pos)-0.1*rangePos-0.001,min(pos)+1.1*rangePos+0.001]);
...
%Get velocity in desired units and find steady-state value
Xv = data(:,4) / motorCPR;  %[rev/s]
settleTime = 2.02; %[s] - choose by visual inspection
SSV = mean(Xv(time > settleTime)); %[rev/s] - steady-state value
SSVvals = ones(length(time))*0.98*SSV;

%Plot and identify settling time
f3 = figure(3); f3.Position = [100 150 600 500];
plot(time,Xv,lineFormat1); hold on; 
plot(time, SSVvals, lineFormat2);
title('Observed Velocity'); 
ylabel('Velocity [rev/s]'); xlabel('Time [s]');
xlim([1.95 2.1]);
set(gcf,'Color','w'); box off;
legend('Observed','SSV','Location','southeast')
